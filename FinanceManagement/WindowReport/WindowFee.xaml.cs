    ...
    public partial class WindowFee: Window
    {
        ...
        private void autoAddFixedFee(string feeName, string feeType, int yearNow)
        {
            ...
            if (gridFee.Items.Count != 0)
            {
                ...
                _ = MessageBox.Show("表示したのデータは「自動」で作成しました。まだ保存しないです。保存したいの時はデータを修正して「保存」ボタンをクリックしてください。", "報告");
            }
            else
            {
                _ = MessageBox.Show("データがありません。", "報告");
            }
        }
        ...
        private void btnInsert_Click(object sender, RoutedEventArgs e)
        {
            ...
            foreach (MMFee temp in observableFees)
            {
            ...
            if (index == -1) // Not have value yet
            {
                MMFee mMFeeSum = new MMFee()
                {
                    ...
                    Month8 = mMFee.Month8,
                    Month9 = mMFee.Month9,
                    Month10 = mMFee.Month10,
                    Month11 = mMFee.Month11,
                    Month12 = mMFee.Month12,
                    Sum = mMFee.Sum,
                    Remark = null
                };

                observableFees.Insert(1, mMFeeSum);

                gridFee.ItemsSource = observableFees;
            }

            if(observableFees.Count == 2) // Not have sum value
            {
                MMFee mMFeeSum = new MMFee()
                {
                    FeeName = FeeName,
                ...
                observableFees.Insert(2, mMFeeSum);
        ...
        {
            autoAddFixedFee(FeeName, cbFeeType.Text, (int)cbYear.SelectedItem);
        }
        ...
        private void Paste(object sender, System.Windows.Input.ExecutedRoutedEventArgs e)
        {
            ...
            foreach (var dataGridCellInfo in dataGridCellInfos)
            {
                ...
                var header = dataGridCellInfo.Column.Header.ToString();

                if (header.Contains("１０月"))
                {
                    mMFee.Month10 = pasteValue;
                }
                else if (header.Contains("１１月"))
                {
                    mMFee.Month11 = pasteValue;
                }
                else if (header.Contains("１２月"))
                {
                    mMFee.Month12 = pasteValue;
                }
                else if (header.Contains("１月"))
                {
                    mMFee.Month1 = pasteValue;
                }
                else if (header.Contains("２月"))
                {
                    mMFee.Month2 = pasteValue;
                }
                else if (header.Contains("３月"))
                {
                    mMFee.Month3 = pasteValue;
                }
                else if (header.Contains("４月"))
                {
                    mMFee.Month4 = pasteValue;
                }
                else if (header.Contains("５月"))
                {
                    mMFee.Month5 = pasteValue;
                }
                else if (header.Contains("６月"))
                {
                    mMFee.Month6 = pasteValue;
                }
                else if (header.Contains("７月"))
                {
                    mMFee.Month7 = pasteValue;
                }
                else if (header.Contains("８月"))
                {
                    mMFee.Month8 = pasteValue;
                }
                else if (header.Contains("９月"))
                {
                    mMFee.Month9 = pasteValue;
                }
            }
        ...
        private void cbNewField_DropDownClosed(object sender, EventArgs e)
        {
            if (sender is ComboBox combobox)
            {
                if (combobox.SelectedValue is MMFeeMaster mMFeeMaster)
                {
                    combobox.Text = mMFeeMaster.Item;
                }
            }
        }
    }
